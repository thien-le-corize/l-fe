...
const LoanActions = () => {
  const actions = useMemo(
    () => [
      ...
      { id: 3, title: "Ví tiền", icon: MyWaltIcon.src, link: "/my-wallet" },
  ...
  return (
    <div className="flex w-3/4 gap-7 my-20 mx-auto justify-center">
      {actions.map((action) => (
        <Link
          key={action.id}
          href={action.link}
          className="hover:opacity-80 flex w-1/4 cursor-pointer bg-sky-600 text-red-50 rounded-3xl px-4 py-8 flex-col justify-center items-center gap-5"
        >
