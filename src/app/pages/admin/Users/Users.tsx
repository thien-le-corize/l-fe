...
export const Users: FC<Props> = ({ adminId }) => {
    ...
    const getUserList = useCallback(async (page = 1) => {
        const res = await httpClient.get('/admin/users', { page, adminId })
        return res.data
    }, [adminId])

    const { data, isLoading, isError, isPlaceholderData } = useQuery({
        queryKey: ['admin', 'users-mana', page + 1],
            ...
            <table className='relative w-full text-left text-sm text-gray-500 shadow-md dark:text-gray-400 rtl:text-right'>
                ...
                <tbody>
                    ...
                    {data.users.map((user: any) => (
                        <tr key={user._id} className='border-b bg-white hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-600'>
                            <td
                                scope='row'
                                ...
                                <div className='relative right-0'>
                                    ...
                                    >
                                        <li>
                                            ...
                                            href={`/admin/user/${user._id}`}><Image width={24} height={24} src={
                                                SeeDetailIcon} alt='' /></Link>
                                        </li>
                                        <li>
                                            <button
                                                className='block w-full px-4 py-2 text-left hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white'
